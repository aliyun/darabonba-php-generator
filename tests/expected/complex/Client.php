...
class Client extends SourceClient {
  ...
  public function complex1($request, $client)
  {
    ...
    while (Dara::shouldRetry($_runtime['retryOptions'], $_context)) {
      ...
      try {
        ...
        $_reqeust->protocol = $this->_protocol;
        $_reqeust->port = 80;
        $_reqeust->method = 'GET';
        $_reqeust->pathname = '/' . $this->_pathname . '';
        $_reqeust->query = SourceClient::query(Dara::merge([
          'date' => '2019',
          'access' => $mapAccess,
          'test' => @$mapVal['test'],
        ], $request->header));
        $_reqeust->body = SourceClient::body();
  ...
  public function Complex2($request, $str, $val)
  {
    ...
    $_reqeust->protocol = 'HTTP';
    $_reqeust->port = 80;
    $_reqeust->method = 'GET';
    $_reqeust->pathname = '/';
    $_reqeust->query = SourceClient::query([
      'date' => '2019',
      'version' => '2019-01-08',
      'protocol' => $_reqeust->protocol,
    ]);
    $_reqeust->body = SourceClient::body();
  ...
  public function Complex3($request)
  {
    $_request = new Request();
    $name = 'complex';
    $_reqeust->protocol = $this->TemplateString();
    $_reqeust->port = 80;
    $_reqeust->method = 'GET';
    $_reqeust->pathname = '/';
    $_reqeust->query = SourceClient::query([
    ...
    $_reqeust->body = SourceClient::body();
    @$_reqeust->headers['host'] = 'hello';
    $_response = Dara::send($_request);

    $temp_str = 'test ' . (string)100 . ' ' . (string)true . ' ' . (string)($_reqeust->port + 4) . ' ' . @$_reqeust->headers['host'] . '';
    $resp = $_response;
