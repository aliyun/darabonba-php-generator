...
${this.output}`;
  ...
  getRealModelName(fullModelName) {
    ...
    this.usedClass.set(modelName, fullModelName);
    return modelName;
  }
  ...
  visitExceptions(ast, filepath, level) {
    ...
    for (let i = 0; i < exs.length; i++) {
      ...
      const modelFilepath = path.join(exDir, `${exceptionName}.php`);
      ...
      if (this.predefined) {
        Object.keys(this.predefined).filter((key) => {
          ...
          const realFullClassName = `${classNamespace}\\Models\\${realFullModelName}`;
...
${this.config.clientName}::main(array_slice($argv, 1));`);
  ...
  getClassNamespace(phpPath) {
    if(phpPath.startsWith(this.outputDir)) {
      const baseDir = path.join(this.outputDir, 'src', path.sep);
  ...
  eachImport(imports, usedModels, filepath, innerModule, level) {
    ...
    if (imports.length > 0) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        const classNamespace = phpPkg.package.split('.').join('\\');
        const className = phpPkg.clientName || 'Client';
  ...
  visitType(ast, level) {
    if (ast.type === 'array') {
      ...
      const realModelName = this.getRealModelName(`${namespace}\\${type}\\${ast.name}`);
      this.emit(realModelName);
      ...
      const realModelName = this.getRealModelName(`${this.namespace}\\${type}\\${ast.lexeme}`);
      this.emit(realModelName);
  ...
  visitFromField(fieldValue, fieldName, key, value, level) {
    ...
    (fieldValue.fieldType && fieldValue.fieldType.type === 'moduleModel')) {
      ...
      const realModelName = this.getRealModelName(`${this.namespace}\\Models\\${fieldValue.lexeme}`);
      this.emit(`${key} = ${realModelName}::fromMap(${value});\n`, level + 1);
  ...
  visitException(exceptionBody, exceptionName, extendOn, level) {
    this.emit(`class ${exceptionName} extends `, level);
  ...
  visitRuntimeAfter(ast, level) {
    ...
    this.emit(`throw ${unretryErrorName}($_context);\n`, level);
  }
